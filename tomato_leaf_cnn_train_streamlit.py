...
3) Scan evaluasi 1 gambar per kelas
4) Web Streamlit untuk upload gambar daun dan prediksi penyakit

Cara pakai:
- Training + evaluasi + scan per kelas:
...
- hasil_tomato_cnn_plant96/pipeline_<kelas>.png
- hasil_tomato_cnn_plant96/scan_per_kelas.csv
"""
...
def save_pipeline_visual(plt, tf, model, x_test, y_test, index, output_dir=OUTPUT_DIR):
    ...
    output_path = output_dir / f"pipeline_{safe_class_name}.png"
...
def run_scan_per_class(plt, tf, model, x_test, y_test):
    print("\n\n============================================================")
    print("SCAN PIPELINE: 1 GAMBAR PER KELAS")
    ...
    scan_csv_path = OUTPUT_DIR / "scan_per_kelas.csv"
    scan_df.to_csv(scan_csv_path, index=False)

    print("\nRingkasan scan per kelas:")
    print(scan_df[["index", "label_asli", "prediksi", "confidence", "status"]])
    print("\nFile ringkasan scan disimpan di:")
    print(scan_csv_path.resolve())
...
def run_training_pipeline():
    ...
    run_scan_per_class(plt, tf, model, x_test, y_test)
    ...
    if uploaded_file is not None:
        ...
        if confidence < 50:
            st.info(
                ...
                "Gunakan hasil sebagai panduan sistem, bukan diagnosis pertanian final."
            )
...
if __name__ == "__main__":
    if is_running_under_streamlit():
        ...
        print("\nCara pakai:")
        print("1. Training + evaluasi + scan per kelas:")
